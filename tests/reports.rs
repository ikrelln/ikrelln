...
const DELAY_REPORT_SAVED_MILLISECONDS: u64 = 300;
...
fn should_create_report() {
    ...
    let reported_span = uuid::Uuid::new_v4().to_string();
    ...
    let req_report = srv.client(
        Method::GET,
        &format!("/api/v1/reports/endpoints/{}", service_name),
    ).finish()
        .unwrap();
    ...
    assert_eq!(data_report.name, service_name);
    assert!(data_report.categories.unwrap().contains_key(&reported_span));
}
