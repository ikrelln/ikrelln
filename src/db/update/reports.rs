...
impl super::DbExecutor {
    ...
    fn update_report_or_create(&mut self, report_db: &ReportDb) -> String {
        ...
        // println!("update_report_or_create: {:?}", report_db);
        match self.find_report(report_db) {
            Some(existing) => {
                // println!("updating existing report");
            ...
            None => {
                // println!("trying to create new report");
                ...
                if could_insert.is_err() {
                    // println!("couldn't insert new report, updating existing one");
